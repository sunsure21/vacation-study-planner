    ...
    function checkTimeConflict(newStart, newEnd, existingSchedules, newCategory) {
        ...
        for (const schedule of existingSchedules) {
            ...
            if (newStartMinutes < blockedEnd && newEndMinutes > blockedStart) {
                return {
                    conflict: true,
                    message: schedule.category === '학원' ? 
                        `학원 시간(${schedule.startTime}-${schedule.endTime})의 앞뒤 1시간은 등록할 수 없습니다.` :
                        schedule.category === '취침' ?
...
function getMBTIDefaultAdvice(mbtiType) {
    const adviceMap = {
        ...
        'ISTJ': {
            characteristics: 'ISTJ는 체계적이고 꾸준한 학습을 선호합니다. 단계별로 차근차근 학습하는 것을 좋아합니다.',
            strategy: '체계적인 학습 계획을 세우고 꾸준히 실행하세요. 반복 학습과 복습을 통해 확실히 익히세요.',
            tips: ['체계적인 계획 수립', '꾸준한 학습 습관', '반복 학습과 복습', '단계별 학습 진행'],
            motivation: '계획대로 꾸준히 진행하며 실력이 향상되는 것을 확인할 때 동기부여됩니다.'
        },
        'ISFJ': {
